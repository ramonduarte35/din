...
class WebhooksService {
    ...
    async extractWithAI(text, userAccounts) {
        ...
        const systemPrompt = `Você é o assistente financeiro inteligente do Din. Sua função é extrair com extrema precisão transações financeiras, contas de destino ou intenções de consulta de mensagens em linguagem natural em português brasileiro.\n\n` +
    ...
    async handleBalanceQuery(user, instance, remoteJid, userAccounts, queryAccountName) {
        ...
        if (queryAccountName) {
            ...
            targetAccount = userAccounts.find((a) => a.name.toLowerCase().includes(qLower) ||
        ...
        if (targetAccount) {
            ...
            const replyMsg = `🏦 *Balanço da Conta: ${targetAccount.name}*\n` +
                ...
                `🟢 *Receitas:* ${(0, currency_js_1.formatBRL)(mIncome)}\n` +
                `🔴 *Despesas:* ${(0, currency_js_1.formatBRL)(mExpense)}\n` +
        ...
        const accountsBalances = await Promise.all(userAccounts.map(async (acc) => {
            const txs = await prisma_js_1.prisma.transaction.findMany({
        ...
        const accountsListText = accountsBalances
            .map((acc) => `• *${acc.name}:* ${(0, currency_js_1.formatBRL)(acc.balance)}${acc.is_default ? ' _(Padrão)_' : ''}`)
            .join('\n');
        const replyMsg = `📊 *Resumo Financeiro - ${currentMonthName}/${now.getFullYear()}*\n` +
            `Olá, *${user.name}*! Aqui está o seu balanço geral:\n\n` +
            `💰 *Saldo Total Consolidado:* *${(0, currency_js_1.formatBRL)(totalBalance)}*\n` +
            `🟢 *Receitas do Mês:* ${(0, currency_js_1.formatBRL)(income)}\n` +
            `🔴 *Despesas do Mês:* ${(0, currency_js_1.formatBRL)(expense)}\n` +
            `━━━━━━━━━━━━━━━━━━\n` +
