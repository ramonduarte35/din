...
class WebhooksService {
    ...
    async processTelegramMessage(payload) {
        ...
        if (message.contact && message.contact.phone_number) {
            ...
            if (user) {
                ...
                const replyMsg = `⚠️ *Conta não encontrada!*\n\n` +
                    `Não encontramos nenhum usuário cadastrado no Dino com o telefone *${(0, phone_js_1.formatPhoneNumberDisplay)(normalizedPhone)}*.\n\n` +
        ...
        if (trimmedText.startsWith('/start') || trimmedText.startsWith('/vincular')) {
            ...
            const replyMsg = `👋 *Olá, ${fromName}! Bem-vindo ao Dino no Telegram!* 💎\n\n` +
        ...
        if (!user) {
            ...
            const replyMsg = `👋 *Olá! Não identificamos uma conta do Dino vinculada a este Telegram.*\n\n` +
    ...
    async processEvolutionMessage(payload) {
        ...
        if (!user) {
            ...
            const replyMsg = `👋 *Olá! Seja bem-vindo ao Dino.*\n\n` +
    ...
    async extractWithAI(text, userAccounts, pendingBills = [], contacts = []) {
        ...
        const systemPrompt = `Você é o Dino, assistente financeiro inteligente. Sua função é extrair com extrema precisão transações financeiras, agendamentos de contas a pagar, liquidações/pagamentos e consultas em português brasileiro.\n\n` +
    ...
    fallbackLocalParser(text, userAccounts, pendingBills = [], contacts = []) {
        ...
        const isRegisterBill = lower.includes('agendar') ||
        ...
        if (isRegisterBill &&
            ...
            const installmentsMatch = lower.match(/(?:em\s*)?(\d{1,3})\s*(?:x|vezes|parcelas?|mensalidades?)\b/i) ||
    ...
    async handleRegisterBill(user, instance, remoteJid, billData, rawText) {
        try {
            ...
            const installmentsInfo = billData.total_installments && billData.total_installments > 1
                ? `🔢 *Parcelamento:* ${billData.total_installments}x de ${(0, currency_js_1.formatBRL)(Number(createdBill.amount))} (próximos meses agendados)\n`
                : '';
    ...
    async handleBalanceQuery(user, instance, remoteJid, userAccounts, queryAccountName) {
        ...
        const accountsBreakdown = userAccounts
            ...
            .join('\n');
        const replyMsg = `📊 *Resumo Financeiro — Dino*\n` +
    ...
    async handleTransactionsRegistration(user, instance, remoteJid, transactions, rawText, userAccounts, origin = client_1.TransactionOrigin.WHATSAPP_TEXT) {
        ...
        for (const item of transactions) {
            ...
            const updatedAccBalance = Number(targetAccount.initial_balance) + accIncome - accExpense;
            let budgetAlertText = '';
            if (type === client_1.TransactionType.EXPENSE && category.id) {
                try {
                    ...
                    if (budgetCheck?.has_budget) {
                        if (budgetCheck.is_exceeded) {
                            ...
                            budgetAlertText = `\n🚨 *Alerta de Orçamento:* Você estourou o teto de *${budgetCheck.category_name}* em ${(0, currency_js_1.formatBRL)(diff)} (${budgetCheck.percentage}% consumido)!`;
                        }
                        else if (budgetCheck.is_warning) {
                            budgetAlertText = `\n⚠️ *Atenção:* Você atingiu ${budgetCheck.percentage}% do orçamento de *${budgetCheck.category_name}* (restam ${(0, currency_js_1.formatBRL)(budgetCheck.remaining_amount)}).`;
                        }
                    }
                }
                catch (e) {
    ...
    async handleUnknownMessage(user, instance, remoteJid, userAccounts) {
        const replyMsg = `🤖 *Assistente Financeiro Dino*\n` +
