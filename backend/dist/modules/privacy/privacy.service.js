...
class PrivacyService {
     ...
     */
    async anonymizeUser(userId, actorId, request) {
        const now = new Date();
        // ① Capturar snapshot antes de modificar (hash do e-mail para diff — não o valor real)
        const userBefore = await prisma_js_1.prisma.user.findUniqueOrThrow({
            where: { id: userId },
        ...
        const dummyPwHash = (0, node_crypto_1.randomBytes)(64).toString('hex');
        // ② Transação atômica
            ...
            if (activeConsents.length > 0) {
                await tx.userConsent.createMany({
                    data: activeConsents.map((c) => ({
                        ...
                        userAgent: request?.headers?.['user-agent'] ?? 'SYSTEM',
                        withdrawnAt: now,
    ...
    async exportUserData(userId) {
        const [user, transactions, bills, accounts, categories, goals, budgets, consents,] = await Promise.all([
            prisma_js_1.prisma.user.findUnique({
                where: { id: userId },
                select: {
            ...
            prisma_js_1.prisma.category.findMany({
                where: { user_id: userId },
                select: { id: true, name: true, type: true, created_at: true },
            }),
            prisma_js_1.prisma.goal.findMany({
                where: { user_id: userId },
                select: {
            ...
            prisma_js_1.prisma.budget.findMany({
                where: { user_id: userId },
