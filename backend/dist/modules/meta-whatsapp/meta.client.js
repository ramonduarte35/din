...
exports.metaClient = exports.MetaCloudApiClient = void 0;
...
class MetaCloudApiClient {
     ...
     */
    async sendText(recipientNumber, message, options) {
        try {
            ...
            console.error('❌ [Meta Cloud API] Erro ao enviar mensagem:', error.response?.data || error.message);
            return false;
        }
    }
    /**
     ...
     */
    async downloadAudioMedia(mediaId, options) {
        try {
            const config = await this.getConfig();
            const accessToken = options?.accessToken || config?.meta_access_token;
            if (!accessToken) {
            ...
            const metaRes = await axios_1.default.get(metadataUrl, {
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                },
                timeout: 10000,
            ...
            const mediaResponse = await axios_1.default.get(downloadUrl, {
                headers: {
                    Authorization: `Bearer ${accessToken}`,
...
exports.MetaCloudApiClient = MetaCloudApiClient;
exports.metaClient = new MetaCloudApiClient();
