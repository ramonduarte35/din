...
class BudgetsService {
     ...
     */
    async getMonthlyBudgets(userId, month, year) {
        const now = new Date();
        ...
        const expenses = await prisma_js_1.prisma.transaction.findMany({
            where: {
                user_id: userId,
        ...
        const allExpenseCategories = await prisma_js_1.prisma.category.findMany({
            where: {
        ...
        const overallPercentage = totalBudgeted > 0 ? Math.round((totalSpentInBudgetedCategories / totalBudgeted) * 100) : 0;
        return {
            month: targetMonth,
            year: targetYear,
            summary: {
    ...
    async upsertBudget(userId, data) {
        ...
        const budget = await prisma_js_1.prisma.budget.upsert({
            where: {
     ...
     */
    async copyFromPreviousMonth(userId, data) {
        const targetMonth = data.target_month;
        const targetYear = data.target_year;
        const prevMonth = targetMonth === 1 ? 12 : targetMonth - 1;
        const prevYear = targetMonth === 1 ? targetYear - 1 : targetYear;
        const prevBudgets = await prisma_js_1.prisma.budget.findMany({
            where: {
                user_id: userId,
                month: prevMonth,
                year: prevYear,
     ...
     */
    async checkCategoryBudgetAlert(userId, categoryId, transactionDate = new Date()) {
        ...
        const totalSpentRes = await prisma_js_1.prisma.transaction.aggregate({
            where: {
                user_id: userId,
                category_id: categoryId,
                type: client_1.TransactionType.EXPENSE,
