...
export function parseCurrencyInput(value: string | number): number {
  ...
  if (raw === 'cinquentão' || raw === 'cinquentao') return 50;
  if (raw === 'vintão' || raw === 'vintao') return 20;
  if (raw === 'dezão' || raw === 'dezao') return 10;
  if (raw === 'cem conto' || raw === 'cem pila') return 100;

  // Tratar multiplicadores expressos em texto (ex: "6 mil", "6k", "1,5 mil", "2 barões", "2 milão")
  const multiplierMatch = raw.match(
    /^(\d+(?:[.,]\d+)?)\s*(?:k|mil|milhao|milhão|milhoes|milhões|barao|barão|baroes|barões|milao|milão|mi)$/i
  );
  if (multiplierMatch) {
...
export function extractAmountFromText(text: string): number {
  ...
  if (/\b(cinquentão|cinquentao)\b/i.test(lower)) return 50;
  if (/\b(vintão|vintao)\b/i.test(lower)) return 20;
  if (/\b(dezão|dezao)\b/i.test(lower)) return 10;
  if (/\b(cem conto|cem pila|cem reais)\b/i.test(lower)) return 100;
  ...
  if (multMatch) {
    ...
    if (!isNaN(base)) {
      ...
      if (unit.startsWith('milh') || unit === 'mi') return Math.abs(base * 1000000);
  ...
  const suffixCurrencyRegex =
    /(\d+(?:[.,]\d+)*)\s*(?:reais|conto|pila|pau|mangos)/i;
