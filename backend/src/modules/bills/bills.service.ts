...
export class BillsService {
  ...
  async listBills(userId: string, query: ListBillsQueryInput) {
    ...
    const formattedBills = bills.map((bill) => {
      ...
      if (bill.status === BillStatus.PENDING && diffDays < 0) {
  ...
  async updateBill(userId: string, id: string, data: UpdateBillInput) {
    ...
    if (data.contact_id !== undefined) updateData.contact = data.contact_id ? { connect: { id: data.contact_id } } : { disconnect: true };
