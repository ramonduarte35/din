...
export class WebhooksService {
   ...
   */
  async processTelegramMessage(payload: any) {
    ...
    if (callbackQuery) {
      ...
      await telegramClient.answerCallbackQuery(cbId);
      ...
      if (cbData === 'cancel_pay') {
        try {
          await redis.del(`pending_pay:${user.id}`);
        } catch (e) { }
      ...
      if (cbData.startsWith('pay_bill:')) {
        ...
        const replyMsg =
          ...
          `💰 *Novo Saldo no ${defaultAccount.name}:* ${formatBRL(updatedAccBalance)}\n` +
    ...
    // Deduplicação com Redis (24 horas)
    try {
      const isDuplicate = await redis.get(`webhook:tele:${fromId}:${messageId}`);
      if (isDuplicate) {
        console.log(`⏭️ [Telegram Webhook] Mensagem duplicada ignorada: ${fromId}:${messageId}`);
        return { status: 'duplicate_ignored' };
      }
      await redis.set(`webhook:tele:${fromId}:${messageId}`, '1', 'EX', 86400);
    } catch (e) {
      // ignore
    }
    ...
    if (message.contact && message.contact.phone_number) {
      ...
      if (!user && normalizedPhone.startsWith('55')) {
        ...
        if (rest.length === 9 && rest.startsWith('9')) {
          user = await prisma.user.findFirst({ where: { phone_number: `55${ddd}${rest.slice(1)}` } });
        } else if (rest.length === 8) {
          user = await prisma.user.findFirst({ where: { phone_number: `55${ddd}9${rest}` } });
        }
      }
    ...
    let origin: TransactionOrigin = TransactionOrigin.TELEGRAM_TEXT;
    ...
    if (message.voice || message.audio) {
      ...
      if (voiceFileId) {
        ...
        if (hasOpenAIConfigured() && openai) {
          try {
            const media = await telegramClient.downloadVoiceAudio(voiceFileId);
            if (media) {
    ...
    if (trimmedText.startsWith('/start') || trimmedText.startsWith('/vincular')) {
      ...
      if (param) {
        ...
        try {
          targetUserId = await redis.get(`tele_link:${param}`);
          if (!targetUserId) {
  ...
  async processEvolutionMessage(payload: any) {
    ...
    const origin: TransactionOrigin = data?.origin || payload?.origin || TransactionOrigin.WHATSAPP_TEXT;
   ...
   */
  async processUserFinancialMessage(
    user: any,
    instance: string,
    remoteJid: string,
    trimmedText: string,
    origin: TransactionOrigin = TransactionOrigin.WHATSAPP_TEXT,
    ...
    try {
      const pendingPayRaw = await redis.get(`pending_pay:${user.id}`);
      if (pendingPayRaw) {
        ...
        let chosenIndex = -1;
        if (/^(?:opção\s*|opcao\s*|numero\s*|número\s*|nº\s*)?1\b|^1️⃣|^primeir[ao]/i.test(lowerText)) chosenIndex = 0;
        else if (/^(?:opção\s*|opcao\s*|numero\s*|número\s*|nº\s*)?2\b|^2️⃣|^segund[ao]/i.test(lowerText)) chosenIndex = 1;
        else if (/^(?:opção\s*|opcao\s*|numero\s*|número\s*|nº\s*)?3\b|^3️⃣|^terceir[ao]/i.test(lowerText)) chosenIndex = 2;
        else if (/^(?:opção\s*|opcao\s*|numero\s*|número\s*|nº\s*)?4\b|^4️⃣|^quart[ao]/i.test(lowerText)) chosenIndex = 3;
        else if (/^(?:opção\s*|opcao\s*|numero\s*|número\s*|nº\s*)?5\b|^5️⃣|^quint[ao]/i.test(lowerText)) chosenIndex = 4;
        else if (/^(?:sim|confirmar|confirma|ok|pode pagar|paguei)$/i.test(lowerText) && pendingPay.bills?.length === 1) chosenIndex = 0;

        // Se o usuário digitou o nome da conta diretamente
        if (chosenIndex === -1 && pendingPay.bills) {
          const matchIdx = pendingPay.bills.findIndex((b: any) =>
            lowerText.includes(b.description.toLowerCase()) || b.description.toLowerCase().includes(lowerText)
          );
          if (matchIdx !== -1) chosenIndex = matchIdx;
        }
        ...
        if (chosenIndex >= 0 && pendingPay.bills && pendingPay.bills[chosenIndex]) {
          ...
          const userAccs = await prisma.account.findMany({ where: { user_id: user.id } });
   ...
   */
  private cleanBillSearchTerm(term?: string): string {
    if (!term) return '';
    return term
      .toLowerCase()
  ...
  private async extractWithAI(
    ...
    const pendingBillsFormatted =
      pendingBills.length > 0
        ? pendingBills
            .map(
              (b) =>
                `- [ID: "${b.id}"] "${b.description}" | Valor: R$ ${Number(b.amount).toFixed(2)} | Vencimento: ${b.due_date ? new Date(b.due_date).toISOString().split('T')[0] : 'N/A'}${b.contact ? ` | Contato Vinculado: "${b.contact.name}"` : ''}`
            )
            .join('\n')
  ...
  private fallbackLocalParser(
    ...
    if (
      ...
      let searchTerm = this.cleanBillSearchTerm(
        text
      ...
      if (!searchTerm) searchTerm = 'conta';
      ...
      if (pendingBills.length > 0 && searchTerm !== 'conta') {
        ...
        if (found) matchedBillId = found.id;
      }
  ...
  private async handlePayBill(
    ...
    try {
      ...
      const activePendingBills =
        pendingBills && pendingBills.length > 0 && pendingBills[0].contact !== undefined
          ? pendingBills
      ...
      if (payData.matched_bill_id) {
        ...
        if (directMatch) matchingBills = [directMatch];
      }
      ...
      if (matchingBills.length === 0) {
        ...
        const searchKeywords = [cleanTerm, contactTerm].filter(
          (t) => Boolean(t) && t !== 'conta' && t !== 'boleto'
        );
        ...
        if (matchingBills.length === 0 && payData.amount && searchKeywords.length === 0) {
          matchingBills = activePendingBills.filter(
      ...
      if (matchingBills.length === 0) {
        const pendingListPreview = activePendingBills
          ...
          .map(
            (b, idx) =>
              `${idx + 1}️⃣ *${b.description}* - ${formatBRL(Number(b.amount))} (Venc: ${formatDateBR(b.due_date)})${b.contact ? ` [${b.contact.name}]` : ''}`
          )
          .join('\n');

        const searchTermDisplay = payData.search_term || payData.contact_name || 'os dados informados';
        const replyMsg =
          `🔍 *Nenhuma conta pendente encontrada para "${searchTermDisplay}".*\n\n` +
          ...
          `• Se isso foi uma despesa avulsa e não uma conta cadastrada, envie: _"Gastei ${payData.amount ? formatBRL(payData.amount) : '50'} com ${searchTermDisplay} no ${targetAccount.name}"_.`;
      ...
      if (matchingBills.length > 1) {
        ...
        try {
          await redis.setex(`pending_pay:${user.id}`, 600, JSON.stringify(cachePayload));
        } catch (e) {
        ...
        const replyMsg =
          `🤔 Encontrei mais de uma conta correspondente. *Qual delas você pagou?*\n\n` +
          `${optionsList}\n\n` +
          ...
          replyMarkup = {
            inline_keyboard: [
              ...matchingBills.slice(0, 5).map((b, idx) => [
                {
                  text: `${idx + 1}. ${b.description} (${formatBRL(Number(b.amount))})`,
