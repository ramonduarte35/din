...
export interface CardProps extends HTMLAttributes<HTMLDivElement> {
  glow?: 'none' | 'emerald' | 'rose' | 'indigo' | 'primary';
  children: ReactNode;
}
...
export function Card({ className, glow = 'none', children, ...props }: CardProps) {
  const glowClasses = {
    none: '',
    primary: 'border-din-primary/40 hover:border-din-primary/60 shadow-lg glow-primary',
  ...
  return (
    <div
      className={cn(
        'rounded-2xl bg-card border border-border backdrop-blur-xl p-5 transition-all duration-200 shadow-xl',
        glowClasses[glow],
        className
      )}
      {...props}
